...
import { logActivity, SalesError } from "@/lib/modules/sales";
...
/** POST /api/v1/rep/prospects/{id}/activities — log a call/email/meeting/note. */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { ctx, employee } = await requireRep();
    const { id } = await params;
    const body = await req.json().catch(() => null);
    const activity = await logActivity(employee.id, id, body, { userId: ctx.userId });
