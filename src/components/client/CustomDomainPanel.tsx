...
export function CustomDomainPanel({
  ...
  return (
    <div className={bare ? "" : "anim-rise mt-6 rounded-2xl border border-stone-200 bg-white p-6 shadow-card"}>
      ...
      {testModeActive && (
        <div className="mt-4 rounded-xl border border-dashed border-violet-300 bg-violet-50 p-3">
          <p className="text-sm font-semibold text-violet-800">🧪 Test mode is on</p>
          <p className="text-xs text-violet-600">Buying a domain is simulated — nothing is registered or charged. Turn it off from the top bar.</p>
        </div>
      )}
