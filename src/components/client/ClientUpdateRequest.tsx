...
/**
 * Live-site update request, gated by the plan's monthly update quota. When the quota is spent it
 * flips to the tier upsell instead of the form (or "resets next month" on the top tier).
 */
  ...
  return (
    <div className="mt-8 rounded-2xl border border-stone-200 bg-white p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-xl text-stone-900">Request a website update</h2>
          <p className="mt-1 text-sm text-stone-500">
            Minor text, style, or section changes to your live site — <strong>{quota.used} of {quota.allowance}</strong>{" "}
            used this month.
          </p>
        </div>
        {!out && !open && !sent && <Button onClick={() => setOpen(true)}>Request an update</Button>}
      </div>

      {sent ? (
        <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-stone-700">
          ✓ Your update is with our team — we&apos;ll review and publish it to your live site shortly.
        </p>
      ) : out ? (
        ...
        <form onSubmit={submit} className="mt-4 grid gap-2">
          <Textarea
            ...
            required
            rows={4}
            placeholder="Describe the change you'd like (e.g. update hours, reword the hero, swap a photo)…"
          />
      ...
      {next && (
        <UpgradeModal open={upsell} onClose={() => setUpsell(false)} toPlan={next.name} reason="more_updates" />
      )}
    </div>
  );
}
