...
// Reveal controller — the PLATFORM owns scroll-reveal so first paint is instant and never
// depends on the Motion CDN import resolving. The contract: generated sites mark elements
// with [data-reveal] but must NOT hide them (no opacity:0 in CSS or JS). This script:
//   • reveals everything already in (or near) the first viewport IMMEDIATELY — the hero
//     never waits on a network import, so there is no "blank for a while" gap;
//   • hides only BELOW-the-fold elements (off-screen, so no visible flash) and fades them
//     in on scroll via IntersectionObserver;
//   • hard-reveals anything still hidden after 1.2s, and reveals all on any error.
// Runs at end of <body>, before the Motion module, so it wins the race on every element.
const REVEAL_CONTROLLER = `<script>(function(){try{var els=[].slice.call(document.querySelectorAll('[data-reveal]'));if(!els.length)return;var show=function(el){el.style.setProperty('opacity','1','important');el.style.setProperty('transform','none','important');};var reduce=matchMedia('(prefers-reduced-motion: reduce)').matches;if(reduce||!('IntersectionObserver'in window)){els.forEach(show);return;}var vh=window.innerHeight||document.documentElement.clientHeight;var hidden=[];els.forEach(function(el){var r=el.getBoundingClientRect();var off=(r.width||r.height)&&r.top>vh*0.92;el.style.transition='opacity .6s ease, transform .6s ease';if(off){el.style.opacity='0';el.style.transform='translateY(22px)';hidden.push(el);}else{show(el);}});var io=new IntersectionObserver(function(en){en.forEach(function(e){if(e.isIntersecting){show(e.target);io.unobserve(e.target);}});},{rootMargin:'0px 0px -8% 0px',threshold:0.05});hidden.forEach(function(el){io.observe(el);});setTimeout(function(){hidden.forEach(function(el){if(getComputedStyle(el).opacity==='0')show(el);});},1200);}catch(e){[].forEach.call(document.querySelectorAll('[data-reveal]'),function(el){el.style.setProperty('opacity','1','important');});}})();</script>`;
...
function withMotionFailsafe(doc: string): string {
  let out = doc.includes("<head>")
    ...
    : doc.includes("</head>")
      ...
      : `${PERF_HEAD}${doc}`;
  out = out.includes("</body>") ? out.replace("</body>", `${REVEAL_CONTROLLER}</body>`) : out + REVEAL_CONTROLLER;
  return out;
}
...
export function serveReviewFrame(html: string, siteToken: string, req: Request): Response {
