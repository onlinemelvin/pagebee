...
export function bookingFeedScript(token: string, meta?: BookingMeta | null, preview?: boolean): string {
  return (
    ...
    "function pbDisable(){[].slice.call(document.querySelectorAll('[data-pb-booking-slot]')).forEach(function(s){var n=s,i,p;for(i=0;i<4;i++){p=n.parentElement;if(p)p.removeChild(n);if(!p||p===document.body||p.hasAttribute('data-page')||/^(MAIN|HEADER|FOOTER|NAV|BODY)$/.test(p.tagName))break;if(p.children.length||(p.textContent||'').replace(/\\s+/g,'').length)break;n=p;}});[].forEach.call(document.querySelectorAll('[data-pb-booking-host]'),function(h){if(!h.closest('[data-pb-booking-slot]'))h.remove();});[].forEach.call(document.querySelectorAll('[data-pb-book-open]'),function(b){b.style.display='none';});pbHideUntaggedBook();}" +
    // Backstop for generations that DIDN'T tag a book/reserve button with data-pb-book-open (so the line
    // above missed it): such a button is dead AND can't be wired, so on a booking-off plan it must be
    // hidden too. Best-effort by label — mirrors the lead form's legacy-CTA detection. Conservative: only
    // <button>s and bare/#-anchor <a>s in the body (skips nav/header/footer chrome, lead CTAs, and real
    // tel:/mailto:/page links, which are legitimate navigation), and short reserve/book-style labels.
    "function pbHideUntaggedBook(){try{var rx=/\\b(book|booking|reserve|reservation|reservations|schedule|appointment|appointments|table)\\b/;[].slice.call(document.querySelectorAll('button,a')).forEach(function(b){if(b.hasAttribute('data-pb-book-open')||b.hasAttribute('data-pb-cta'))return;if(b.closest&&b.closest('nav,header,footer,[data-pb-booking-host],.pb-bk-section'))return;var href=(b.getAttribute&&b.getAttribute('href'))||'';if(b.tagName==='A'&&href&&href.charAt(0)!=='#')return;var t=(b.textContent||'').replace(/\\s+/g,' ').trim().toLowerCase();if(!t||t.length>32)return;if(rx.test(t))b.style.display='none';});}catch(e){}}" +
